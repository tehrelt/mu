...
func SetupTracer(ctx context.Context, cfg *config.Config) (trace.Tracer, error) {
	exporter, err := newExporter(ctx, cfg.Jaeger.Endpoint)
	if err != nil {
		return nil, err
	}

	provider, err := newProvider(exporter, cfg.App.Name)
	if err != nil {
		return nil, err
	}
