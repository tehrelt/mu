...
import (
	...
	"github.com/tehrelt/mu/ticket-service/internal/transport/grpc"
...
func New(ctx context.Context) (*App, func(), error) {
	...
	client, cleanup, err := _mongo(ctx, configConfig)
	if err != nil {
		return nil, nil, err
	}
	storage, err := _ticketstorage(configConfig, client)
	if err != nil {
		cleanup()
		return nil, nil, err
	}
	...
	server := grpc.New(configConfig, storage, broker, tracer)
	v := _servers(server)
	app := newApp(v)
	return app, func() {
		cleanup2()
